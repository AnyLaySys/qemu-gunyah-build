...
popd
cd "$outDir"
"$qvmSrc/configure" --prefix="$prefix" --host-cc="$hostCC" --cross-prefix="${targetTriple}-" --cc="$CC" --cxx="$CXX" --extra-cflags="$CFLAGS" --extra-ldflags="$LDFLAGS -lX11 -lXext -lxcb -lXau -lXdmcp -lXrender -lX11-xcb -landroid-shmem -lEGL -lGLESv2" -Dvirglrenderer=enabled -Dgunyah=enabled -Dlinux_io_uring=enabled -Dpixman=enabled "${displayOpts[@]}" --target-list="aarch64-softmmu"
